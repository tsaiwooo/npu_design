# npu_design
desing NPU which MAC unit and elementwize run layer by layer, 每次一個op算完都會存回sram, 下一個op則會再去sram拿資料
...
先進到pc03自己的~/
```
git clone -b layer_by_layer git@github.com:tsaiwooo/npu_design.git
```
## Run
去Makefile裡面確認是否是跑這tb
```
TESTBENCH_FILE = tb_model.sv
```
[從這](https://drive.google.com/drive/folders/1BW6BpjYJSXzhFQMXOsQQcrQkeGRxF0vk)選model, 推薦先選part-efficient跑, 其他有些會跑比較久(1~2天以上), 點進去選baseline的數據, 複製到metadata.txt後下`make`就可以了
### tb_model.sv flow
